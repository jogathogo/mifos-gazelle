...
# deployer.sh -- the main Mifos Gazelle deployer script (Ubuntu + macOS compatible)
...
deployBPMNs() {
  ...
  local bpms_to_deploy=$(ls -l "$BPMNS_DIR"/*.bpmn 2>/dev/null | wc -l | tr -d ' ')
...
function cloneRepo() {
  ...
  # macOS-compatible chown
  if [[ "$MACHINE" == "Mac" ]]; then
      chown -R "$k8s_user" "$target_directory" 2>/dev/null || true
  else
  ...
  if [ -d "$repo_path" ]; then
    ...
    if [[ "$MACHINE" == "Mac" ]]; then
        sudo -u "$k8s_user" git fetch origin "$branch" >> /dev/null 2>&1
    else
    ...
    if [ "$LOCAL" != "$REMOTE" ]; then
        ...
        rm -rf "$repo_path"
        if [[ "$MACHINE" == "Mac" ]]; then
            sudo -u "$k8s_user" git clone -b "$branch" "$repo_link" "$repo_path" >> /dev/null 2>&1
        else
    ...
    echo "$namespaces" | while read -r namespace; do
        ...
        if [[ $namespace == "default" ]]; then
          ...
          if [[ "$deployment_available" == "True" ]]; then
            ...
            if [[ "$MACHINE" == "Mac" ]]; then
                sudo -u "$k8s_user" bash -c "curl -sL https://github.com/prometheus-operator/prometheus-operator/releases/download/${LATEST}/bundle.yaml | kubectl -n default delete -f -" >/dev/null 2>&1
            else
...
function deployHelmChartFromDir() {
  ...
  if [ -n "$values_file" ]; then
      ...
      if [[ "$MACHINE" == "Mac" ]]; then
          sudo -u "$k8s_user" helm install --wait --timeout 600s "$release_name" "$chart_dir" -n "$namespace" -f "$values_file"
      else
      ...
      if [[ "$MACHINE" == "Mac" ]]; then
          sudo -u "$k8s_user" helm install --wait --timeout 600s "$release_name" "$chart_dir" -n "$namespace"
      else
          su - "$k8s_user" -c "helm install --wait --timeout 600s $release_name $chart_dir -n $namespace"
      fi
  fi

  resource_count=$(kubectl get pods -n "$namespace" --ignore-not-found=true 2>/dev/null | grep -v "No resources found" | wc -l | tr -d ' ')
...
function preparePaymentHubChart(){
  ...
  function ensureHelmDeps() {
    ...
    if [[ -f "$chartPath/Chart.lock" && -s "$chartPath/Chart.lock" ]]; then
      ...
      local actual=$(find "$chartPath/charts" -maxdepth 1 -name '*.tgz' 2>/dev/null | wc -l | tr -d ' ')

      if [[ $actual -ge $expected && $expected -gt 0 ]]; then
        echo "      charts/ already populated ($actual/$expected) → running helm dep build"
        if [[ "$MACHINE" == "Mac" ]]; then
            sudo -u "$k8s_user" bash -c "cd $chartPath && helm dep build" >> /dev/null 2>&1
        else
        ...
        if [[ "$MACHINE" == "Mac" ]]; then
            sudo -u "$k8s_user" bash -c "cd $chartPath && helm dep update" >> /dev/null 2>&1
        else
    ...
    if [[ "$MACHINE" == "Mac" ]]; then
        sudo -u "$k8s_user" bash -c "cd $chartPath && helm repo index ." >> /dev/null 2>&1
    else
...
function checkPHEEDependencies() {
  ...
  LATEST=$(curl -s https://api.github.com/repos/prometheus-operator/prometheus-operator/releases/latest | jq -cr .tag_name)
  if [[ "$MACHINE" == "Mac" ]]; then
      sudo -u "$k8s_user" bash -c "curl -sL https://github.com/prometheus-operator/prometheus-operator/releases/download/${LATEST}/bundle.yaml | kubectl create -f -" >/dev/null 2>&1
  else
...
function deployPhHelmChartFromDir(){
  ...
  if [ "$debug" = true ]; then
    ...
    if [[ "$MACHINE" == "Mac" ]]; then
        sudo -u "$k8s_user" bash -c "$helm_cmd"
    else
    ...
    if [[ "$MACHINE" == "Mac" ]]; then
        output=$(sudo -u "$k8s_user" bash -c "$helm_cmd" 2>&1)
    else
  ...
  if [[ "$MACHINE" == "Mac" ]]; then
      sudo -u "$k8s_user" helm status "$releaseName" -n "$namespace" > /tmp/helm_status_output 2>&1
  else
...
function deployInfrastructure () {
  ...
  if [[ "$MACHINE" == "Mac" ]]; then
      sudo -u "$k8s_user" bash -c "cd $INFRA_CHART_DIR && helm dep update" >> /dev/null 2>&1
  else
...
function applyKubeManifests() {
  ...
  for file in "$directory"/*persistence*.yaml; do
    if [ -f "$file" ]; then
      if [[ "$MACHINE" == "Mac" ]]; then
          sudo -u "$k8s_user" kubectl apply -f "$file" -n "$namespace" >> /dev/null 2>&1
      else
...
function addKubeConfig(){
  ...
  if [ ! -d "$K8sConfigDir" ]; then
      if [[ "$MACHINE" == "Mac" ]]; then
          sudo -u "$k8s_user" mkdir -p "$K8sConfigDir"
      else
  ...
  if [[ "$MACHINE" == "Mac" ]]; then
      sudo -u "$k8s_user" cp "$k8s_user_home/k3s.yaml" "$K8sConfigDir/config"
  else
...
function vnext_restore_demo_data {
  ...
  kubectl cp  "$mongo_data_dir/$mongo_dump_file" "$mongopod:/tmp/mongodump.gz" --namespace "$namespace" >/dev/null 2>&1
